...
import { Clock, MapPin, Smile, TrendingUp, Calendar, CheckCircle, Loader2 } from "lucide-react"
...
export default function CheckInPage() {
  ...
  useEffect(() => {
    if (user && !authLoading && ["team_member", "team_leader"].includes(user.role)) {
      fetchTodayCheckin()
      fetchRecentCheckins()
    }
  }, [user, authLoading])
  ...
  // Show access denied if user is not authenticated or doesn't have proper role
      ...
      <div className="flex items-center justify-center min-h-[400px]">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              {!user
                ? "Please log in to access the daily check-in feature."
                : "Daily check-in is only available for team members and team leaders."}
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
